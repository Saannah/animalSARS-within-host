...
calculate_shannon_for_1_sample = function(rownum){
  ...
  if(length(which(abundance_vector < abundance_threshold)) > 0){
    abundance_vector = abundance_vector[-which(abundance_vector < abundance_threshold)]
  }
...
write.table(all, file = 
              "/users/sana/Documents/AIM3/021-full_pipeline/1-metrics/metrics_strict_thresholds_min50_5percent.tsv",
            sep = "\t",
            row.names = FALSE)
